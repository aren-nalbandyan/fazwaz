    ...
    public static function generateDummyData(){
        ...
        for($i = 0; $i < 1000; $i++){
            ...
            for($k = 0; $k < 100; $k ++){
                ...
                if($bedroom === 2 && $status[$statusRand]["name"] === "Active" && $forSale && $propertyTypes[$randomPropertyType]["name"] === "Condo"){
                    if($sale_bedrooms_count > 3000){
                        do{
                            ...
                            $forSale = $faker->numberBetween(0, 1);
                            $randomPropertyType = $faker->numberBetween(0, count($propertyTypes) - 1);
                        } while ($bedroom === 2 && $status[$statusRand]["name"] === "Active" && $forSale && $propertyTypes[$randomPropertyType]["name"] === "Condo");
                    }
                    else $sale_bedrooms_count ++;
                }
                if($regions[$randomRegion]->id === $fourthRegion && $status[$statusRand]["name"] === "Inactive" && $propertyTypes[$randomPropertyType]["name"] === "House" && $forRent){
                    do{
                        $forRent = $faker->numberBetween(0, 1);
                        $randomRegion = $faker->numberBetween(0, count($regions) - 1);
                        $randomPropertyType = $faker->numberBetween(0, count($propertyTypes) - 1);
                    ...
                    } while($regions[$randomRegion]->id === $fourthRegion && $status[$statusRand]["name"] === "Inactive" && $propertyTypes[$randomPropertyType]["name"] === "House" && $forRent);
                }
        ...
        $two_bedrooms_count = Property::where('bedroom', 2)->where("status_id", $activeStatusId)->where("for_sale", 1)->where("property_type", $condoPropertyTypeId)->count();
        if($two_bedrooms_count < 3000) {
            $limit = 3000 - $two_bedrooms_count;
            $not_two_bedrooms = Property::where('bedroom', '<>', 2)->orWhere("status_id", "<>", $activeStatusId)->orWhere("for_sale", 0)->where("property_type", "<>", $condoPropertyTypeId)->limit($limit)->get();
        ...
        $propertyProjectCount = Property::where("project_id", $randProject->id)->where("status_id", $inactiveStatusId)->where("property_type", $housePropertyTypeId)->where("region_id", $fourthRegion)->count();
        if($propertyProjectCount < 2001){
            $properties = Property::where('bedroom', '<>', 2)->orWhere("project_id", "<>", $randProject->id)->orWhere('region_id', $fourthRegion)->orWhere("status_id", '<>', $inactiveStatusId)->orWhere("property_type", '<>', $housePropertyTypeId)->limit(2001 - $propertyProjectCount)->get();
